...
class NewHelpCommand(commands.HelpCommand):
    ...
    async def send_command_help(self, command):
        ...
        e = discord.Embed(color=0x8a5353)
        e.set_author(name=command.name,
                     icon_url=self.context.bot.user.avatar_url)
        params = []
        for param in command.clean_params:
            params.append(f"[{param}]")
        e.add_field(name=_(self.context.lang, "Użycie"), value='`' + f"{command.name} {' '.join(params)}" + '`', inline=False)
        try:
            e.add_field(name=_(self.context.lang, "Twoje wymagane uprawnienia"), value='`' + ', '.join(command.callback.required_permissions) + '`', inline=False)
    ...
    async def send_bot_help(self, mapping):
        ...
        e.set_footer(text=
                     "🌹 " + _(self.context.lang, "{} komend razem. Bot stworzony przez Style. Dołącz do serwera pomocy z botem w razie problemów użyj: {}support. {}help [cmd_or_ext] - aby dowiedzieć się więcej o komendzie lub module.").format(len(self.context.bot.commands), self.clean_prefix, self.clean_prefix))
        await self.context.send(embed=e)
