...
class Pages:
    ...
    def prepare_embed(self, entries, page, *, first=False, index_allowed=False):
        p = []
        ...
        if self.maximum_pages > 1:
            if self.show_entry_count:
                text = f'{page}/{self.maximum_pages} ({len(self.entries)})'
            else:
    ...
    def get_embed(self, entries, page, *, first=False, index_allowed=False):
        self.prepare_embed(entries, page, first=first, index_allowed=index_allowed)
        return self.embed

    async def show_page(self, page, *, first=False, index_allowed=False):
    ...
    async def numbered_page(self):
        ...
        to_delete = []
        to_delete.append(await self.channel.send('Na jaką stronę chcesz iść?'))
        ...
        try:
            ...
            to_delete.append(await self.channel.send('To zajeło zbyt długo.'))
            ...
            if page != 0 and page <= self.maximum_pages:
                ...
                to_delete.append(await self.channel.send(f'Zła strona podana. ({page}/{self.maximum_pages})'))
                await asyncio.sleep(5)
    ...
    async def show_help(self):
        ...
        messages.append('To pozwoli w interaktywny sposób zobaczyć strony z '
    ...
    async def paginate(self, index_allowed=False):
        """Actually paginate the entries and run the interactive loop if necessary."""
        first_page = self.show_page(1, first=True, index_allowed=index_allowed)
        if not self.paginating:
            await first_page
        else:
...
class FieldPages(Pages):
    ...
    async def show_page(self, page, *, first=False):
        ...
        for key, value in entries:
            self.embed.add_field(name=f"**{key}**", value=value, inline=False)

        if self.maximum_pages > 1:
            if self.show_entry_count:
                text = f'Strona {page}/{self.maximum_pages} ({len(self.entries)})'
            else:
...
class HelpPaginator(Pages):
    ...
    async def show_page(self, page, *, first=False):
        ...
        if hasattr(self, '_is_bot'):
            value = 'Aby dostać więcej pomocy: https://discord.gg/EZ3TsYY'
            self.embed.add_field(name='Support', value=value, inline=False)

        self.embed.set_footer(text=f'Użyj "{self.prefix}help command" po więcej informacji.')
        ...
        if self.maximum_pages:
            self.embed.set_author(name=f'{page}/{self.maximum_pages} ({self.total})')
