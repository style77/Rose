    ...
    async def on_message(self, message):
        if self.is_afk(message.author.id):
            if message.author.id == message.author.id:
                ...
                await self.bot.pg_con.execute("DELETE FROM afk WHERE user_id = $1", message.author.id)

        for member in message.mentions:

            data = await self.bot.pg_con.fetch("SELECT * FROM afk WHERE user_id = $1", member.id)
