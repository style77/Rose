    ...
    async def on_raw_reaction_add(self, payload):  # todo custom shade
        ...
        if payload.emoji.name == emoji or str(payload.emoji) == emoji:
            ...
            if star:
                ...
                reactions = [reaction for reaction in msg.reactions if reaction.emoji == emoji or reaction.emoji.name == emoji]
                ...
                bot_message = await starboard.send(content=f"{emoji} **1** {msg.channel.mention} ID: {msg.id}", embed=e)
